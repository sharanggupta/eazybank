...
cd customergateway && ./mvnw clean test
```
...
curl http://CLUSTER_IP:NODEPORT/swagger-ui.html  # Test customergateway
```
...
```bash
# Get NodePort
NODEPORT=$(kubectl get svc customergateway -n eazybank-staging -o jsonpath='{.spec.ports[0].nodePort}')
